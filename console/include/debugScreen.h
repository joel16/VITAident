...
#define SCREEN_GLYPH_W  (16)
#define SCREEN_GLYPH_H  (16)
...
static size_t psvDebugScreenEscape(const char *str)
{
	...
	{	
		...
		else if (str[i] == 'm')
		{
			...
			{
				switch(params[j] / 10)
				{/*bold,dim,underline,blink,invert,hidden => unsupported yet */
...
int psvDebugScreenPuts(const char * text)
{
	...
	for (c = 0; text[c] != '\0' ; c++) 
	{
		...
		for (i = l = 0; i < (SCREEN_GLYPH_W / 2); i++, l += (SCREEN_GLYPH_W / 2), font++)
		{
			vram_ptr  = vram;
			for (j = 0; j < (SCREEN_GLYPH_W / 2); j++) 
			{
				if ((*font & (128 >> j)))
				{
					*(SceUInt32 *)(vram_ptr) = psvDebugScreenColorFg;
					*(SceUInt32 *)(vram_ptr + 1) = psvDebugScreenColorFg;
					*(SceUInt32 *)(vram_ptr + SCREEN_FB_WIDTH) = psvDebugScreenColorFg;
					*(SceUInt32 *)(vram_ptr + SCREEN_FB_WIDTH + 1) = psvDebugScreenColorFg;
				}
				else
				{
					*(SceUInt32 *)(vram_ptr) = psvDebugScreenColorBg;
					*(SceUInt32 *)(vram_ptr + 1) = psvDebugScreenColorBg;
					*(SceUInt32 *)(vram_ptr + SCREEN_FB_WIDTH) = psvDebugScreenColorBg;
					*(SceUInt32 *)(vram_ptr + SCREEN_FB_WIDTH + 1) = psvDebugScreenColorBg;
				}
				vram_ptr += 2;
			}
			vram += (2 * SCREEN_FB_WIDTH);
		}
...
SceInt printStr(SceBool printStar, uint32_t color, char message[250], char * info, ...)
{
	char buf[512];
	...
	psvDebugScreenPrintf("%s", message); 
